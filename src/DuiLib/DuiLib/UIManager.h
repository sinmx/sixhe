...
public:
	...
	void SetClientSize(int cx, int cy);//添加设置客户端大小方法，用于替换语言时，可以自动设置窗口大小
    SIZE GetInitSize();
	...
	CControlUI* FindFirstParentByClass(CControlUI* pCurrentControl, LPCTSTR pstrClass);
	...
	void SetWndContainerCallback(IUIWndContainerCallback* pCallback) {m_pWndContainerCallback = pCallback;}
	IUIWndContainerCallback* GetWndContainerCallback() {return m_pWndContainerCallback;}
...
private:
	...
	//
	IUIWndContainerCallback* m_pWndContainerCallback;		//窗口类容器回调
