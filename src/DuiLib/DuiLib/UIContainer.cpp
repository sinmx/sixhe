...
void CContainerUI::SetFloatPos(int iIndex)
{
	...
	//子控件float属性为true情况下，当父控件的大小为0时，不设置子控件位置；
	//原因：最小化窗口时，主窗口大小为160*27，导致很多容器控件大小为0，
	//       通过setpos使得容器子控件的szXY.cx，szXY.cy（SetPos中处理）为负，
	//		 从而在SetFloatPos中初始化子控件位置时，以右下角为基点，使控件偏移原始位置
