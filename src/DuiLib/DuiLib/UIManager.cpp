...
void CPaintManagerUI::SetClientSize(int cx, int cy)
{
	if( m_hWndPaint != NULL ) {
    ...
    case WM_MOUSEHOVER:
        {
			...
			::SendMessage(m_hwndTooltip, TTM_SETMAXTIPWIDTH, 0, (LPARAM) 500);//修改tooltip最大显示宽度为500
...
{
	CStdString strFontName = pStrFontName;	
	while(!strFontName.IsEmpty())
	{
		CStdString strFont = strFontName;
		int nIndex = strFontName.Find(_T(","));
		if (nIndex != -1)
		{
			strFont = strFontName.Left(nIndex);
			strFontName = strFontName.Mid(nIndex+1);
		}
		else
		{
			strFontName = _T("");
		...
		if( hFont == NULL ) continue;

		::DeleteObject(m_DefaultFontInfo.hFont);
		m_DefaultFontInfo.hFont = hFont;
		m_DefaultFontInfo.sFontName = strFont;
...
CControlUI* CPaintManagerUI::FindFirstParentByClass(CControlUI* pControl, LPCTSTR pstrClass)
{
	CControlUI* pParent = pControl->GetParent();
	BOOL bFind = FALSE;
	while (pParent != NULL)
	{
		CStdString strTemp = pParent->GetClass();
		if(strTemp == pstrClass)
		{
			bFind = TRUE;
			break;
		}
