...
HRESULT DCreateTextServices(IUnknown *punkOuter, ITextHost *pITextHost, IUnknown **ppUnk)
{
	//
	//在X64下，VS未提供Riched20.lib，于是只能采用动态加载msftedit.dll
	//
	static HMODULE m_hMSFTEdit = NULL;  //微软RichEdit的模块实例句柄
	if (m_hMSFTEdit == NULL)
	{
		m_hMSFTEdit = LoadLibrary(_T("msftedit.dll")); //在此加载，不释放，程序退出自动释放, msftedit.dll 在xpsp1以后使用
		if (m_hMSFTEdit == NULL)
		{
			m_hMSFTEdit = LoadLibrary(_T("riched20.dll")); //在此加载，不释放，程序退出自动释放，win2k下需要加载riched20.dll
		}
	}
	ASSERT(m_hMSFTEdit != NULL);
	if (m_hMSFTEdit == NULL)  //load fail
	{
		return S_FALSE; 
	}	
	
	PCreateTextServices TextServicesProc = (PCreateTextServices)GetProcAddress(m_hMSFTEdit, "CreateTextServices");
	if (TextServicesProc == NULL)
	{
		return S_FALSE;
	}

	return TextServicesProc(punkOuter, pITextHost, ppUnk);	
}
...
BOOL CTxtWinHost::Init(CRichEditUI *re, const CREATESTRUCT *pcs)
{
    ...
    if(FAILED(DCreateTextServices(NULL, this, &pUnk)))
        goto err;
...
void CRichEditUI::SetWinStyle(LONG lStyle)
{
	...
	if( m_lTwhStyle & ES_LEFT)
	{
		wAlignment = PFA_LEFT;
	}
	else if(m_lTwhStyle & ES_CENTER)
	{
		wAlignment = PFA_CENTER;
	}
	else if(m_lTwhStyle & ES_RIGHT)
	{
		wAlignment = PFA_RIGHT;
	}
	m_pTwh->SetDefaultAlign(wAlignment);
}
...
#else
    int iLen = (int)_tcslen(lpszNewText);
    LPWSTR lpText = new WCHAR[iLen + 1];
    ::ZeroMemory(lpText, (iLen + 1) * sizeof(WCHAR));
