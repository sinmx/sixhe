...
void COptionUI::Selected(bool bSelected, bool bNeedNotifyEvent)
{
...
void CEditWnd::Init(CEditUI* pOwner)
{
	...
	if( m_pOwner->IsNumberMode() ) uStyle |= ES_NUMBER;		//add
	if (m_pOwner->GetTextStyle() & DT_RIGHT) uStyle |= ES_RIGHT;//add
	if (m_pOwner->GetTextStyle() & DT_CENTER) uStyle |= ES_CENTER;//add
...
LRESULT CEditWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	else if (uMsg == WM_CHAR)
	{
		...
		{
			m_pOwner->GetManager()->SendNotify(m_pOwner, _T("char"), wParam);
		}
...
CEditUI::CEditUI() : m_pWindow(NULL), m_uMaxChar(255), m_bReadOnly(false), m_bNumberMode(false),
m_bPasswordMode(false), m_cPasswordChar(_T('*')), m_uButtonState(0), m_dwEditbkColor(0xFFFFFFFF), m_dwExampleTextColor(0xff000000),
m_dwExampleBkColor(0xffe6e6e6)
{
    SetTextPadding(CRect(4, 0, 4, 0));
...
void CEditUI::SetText(LPCTSTR pstrText)
{
    ...
    if (m_pWindow != NULL)
	{
		...
		Edit_SetSel(*m_pWindow, m_sText.GetLength(), m_sText.GetLength());
	}
...
void CEditUI::SetNumberMode(bool bNumberMode)
{
	if (m_bNumberMode == bNumberMode) return;
	m_bNumberMode = bNumberMode;
	if( m_pWindow != NULL ) 
	{
		LONG styleValue = ::GetWindowLong(*m_pWindow, GWL_STYLE);
		if (m_bNumberMode) styleValue |= ES_NUMBER;
		else styleValue &= ~ES_NUMBER;
		::SetWindowLong(*m_pWindow, GWL_STYLE, styleValue);
	}
	Invalidate();
}
bool CEditUI::IsNumberMode() const
{
	return m_bNumberMode;
}
...
void CEditUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if( _tcscmp(pstrName, _T("number")) == 0 ) SetNumberMode(_tcscmp(pstrValue, _T("true")) == 0);//add
	...
	else if (_tcscmp(pstrName, _T("exampletext")) == 0)
	{
		SetExampleText(pstrValue);
	}
	else if (_tcscmp(pstrName, _T("exampletextcolor")) == 0)
	{
		if( *pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
		LPTSTR pstr = NULL;
		DWORD clrColor = _tcstoul(pstrValue, &pstr, 16);
		SetExampleTextColor(clrColor);
	}
	else if (_tcscmp(pstrName, _T("examplebkcolor")) == 0)
	{
		if( *pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
		LPTSTR pstr = NULL;
		DWORD clrColor = _tcstoul(pstrValue, &pstr, 16);
		SetExampleBkColor(clrColor);
	}
    else CLabelUI::SetAttribute(pstrName, pstrValue);
}
